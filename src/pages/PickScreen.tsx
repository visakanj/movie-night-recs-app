...
const MOVIES: Movie[] = [{
  ...
  id: '2',
  title: 'Poor Things',
  poster: 'https://image.tmdb.org/t/p/original/kCGlIMZRfwROtH6CNq7zE LSTn9m.jpg',
  genre: 'Comedy',
  duration: '2h 21m',
  rating: '8.1',
...
export function PickScreen() {
  ...
  // Constants
  const ITEM_WIDTH = 220; // Width of poster
  const ITEM_HEIGHT = 330; // Height of poster
  const ITEM_GAP = 24; // Gap between posters
  const TOTAL_ITEM_WIDTH = ITEM_WIDTH + ITEM_GAP;
  // Determine winner ahead of time for the demo
  useEffect(() => {
    // Pick a random winner from the middle set of the duplicated array to ensure scrolling happens
  ...
  return <div className="min-h-screen bg-background pb-28 animate-fade-in overflow-hidden flex flex-col">
      ...
      <main className="flex-1 pt-20 flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div ref={containerRef} className="relative w-full h-[400px] flex items-center overflow-hidden bg-surface/30 border-y border-white/5 backdrop-blur-sm shadow-inner">
          ...
          {/* Center Line Indicator (optional, for debugging or visual guide) */}
          {/* <div className="absolute top-0 bottom-0 left-1/2 w-px bg-red-500/30 z-50" /> */}

          {/* The Strip */}
          <motion.div animate={controls} initial={{
          x: 0
        }} className="flex items-center absolute left-0 pl-[50vw] md:pl-[50%]" // Start with first item near center
        style={{
          marginLeft: -ITEM_WIDTH / 2
        }} // Adjust so first item is perfectly centered initially if x=0
        >
            ...
            return <motion.div key={`${movie.id}-${index}`} className="relative flex-shrink-0 flex justify-center items-center" style={{
              ...
              damping: 15
                  ...
                  <div className={`
                    ...
                    {/* Dark overlay for non-winners during reveal */}
          ...
          duration: 0.8
            ...
            delay: 0.7
              ...
              <p className="text-sm text-text-tertiary line-clamp-3 leading-relaxed">
                {winningMovie.description}
              </p>
            </motion.div>}
        </AnimatePresence>

        {/* Hold Button */}
        {state !== 'revealing' && state !== 'finished' && <div className="mt-12 relative">
            {/* Progress Ring */}
            <svg className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 -rotate-90 pointer-events-none">
              ...
              <motion.circle cx="48" cy="48" r="46" stroke="currentColor" strokeWidth="4" fill="transparent" className="text-accent" strokeDasharray="289.02" // 2 * PI * 46
          strokeDashoffset={289.02 - 289.02 * holdProgress / 100} strokeLinecap="round" />
            </svg>
